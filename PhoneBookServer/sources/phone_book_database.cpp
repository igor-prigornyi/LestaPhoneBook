...
// Подключим библиотеку fstream для работы с потоком ввода-вывода в файл, библиотеку cmath для
// использования математических функций (требуется функция логарифма) и библиотеку algorithm
// для использования стандартных алгоритмов
#include <fstream>
#include <cmath>
#include <algorithm>
...
void PhoneBookDatabase::LoadFromFile() {
    ...
    for(size_t i = 0; i < number_of_records; ++i) {
        ...
        // Замечание: если в каком-либо поле (имя/фамилия/отчество/номер телефона/заметка) есть
        // символы кавычек, это приведёт к неправильному чтению из файла. Эту ситуацию можно
        // купировать, заменяя символы кавычек на &quot при записи и обратно при чтении
...
void PhoneBookDatabase::SaveToFile() const {
    ...
    database_file << "<records_count=\""s << records_.size() << "\" last_record_id=\""s << last_record_id_ << "\">"s << endl;

    // Записываем каждую запись и базы данных в файл
    for(const auto& [record_id, record] : records_) {
        database_file <<   "<id=\""s         << record_id         << 
                         "\" name=\""s       << record.name       <<
                         "\" surname=\""s    << record.surname    <<
                         "\" patronymic=\""s << record.patronymic <<
...
size_t PhoneBookDatabase::AddRecordById(size_t record_id, const Record& record) {
    ...
    for (string_view word : note_words) {
        ...
        // будем добавлять каждый раз к значению TF слова (вначале инициализировано нулём) анее вычисленную константу
...
size_t PhoneBookDatabase::DeleteRecordById(size_t record_id) {
    ...
    // Удаляем запись из базы даных. Вначале стоит удалить упоминание записи из тех словарей, где
    // ключом является string_view, который ссылается на строки из контейнера records_, а затем
    // уже сам контейнер records_, иначе зависимые string_view инвалидируются и удаление будет
    // невозможно
...
optional<vector<PhoneBookDatabase::RecordWithId>> PhoneBookDatabase::FindRecordsByNote(const string& note) const {
    ...
    for (string_view word : words) {
        ...
        // перебераем в цикле все записи, где встречается конкретное слово
    ...
    // Сортируем найденные записеи по убыванию релевантности по TF-IDF

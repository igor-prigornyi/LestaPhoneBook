...
void PhoneBookDatabase::LoadFromFile() {
    ...
    // Замечание: для того, чтобы избежать проблем с кавычками в имени/фамилии/отчестве/номере телефона/заметке,
    // при сохранении данных из базы в файл все кавычки заменяются на "&quot;", а при чтении даных из файла в
...
size_t PhoneBookDatabase::DeleteRecordById(size_t record_id) {
    ...
    // Удаляем запись из базы даных. Вначале стоит удалить упоминание записи из тех словарей, где ключом
